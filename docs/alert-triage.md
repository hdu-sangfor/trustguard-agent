...
Orchestrator (Workflow Engine)
  └─ alert_triage.graph_core — LangGraph 8 节点状态图
       ...
       ├─ query_rag       → RAG Service (placeholder)
       ├─ make_decision   → LLM
       ├─ validate_decision
       ├─ persist_result
       └─ recommend_action
...
- **RAG 降级**: RAG 不可用时继续基于 XDR 原始证据研判，不阻塞流程
...
| `RAG_BASE_URL` | `http://localhost:18980` | RAG 服务地址 |
...
[❌] Agent 调用 trustguard-rag 并保留 citation      ← 占位
[✅] 结构化 Schema (Pydantic)
