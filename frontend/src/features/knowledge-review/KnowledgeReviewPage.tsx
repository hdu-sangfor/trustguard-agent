...
function itemStatus(status: ApiKnowledgeCrawlerReviewItem["status"]): string {
  return { pending: "待审核", processing: "处理中", approved: "已通过", rejected: "已驳回" }[status];
}
...
export default function KnowledgeReviewPage() {
  ...
  const loadReview = useCallback(async () => {
    ...
    try {
      ...
      setSelected((current) => new Set([...current].filter((id) => result.items.some((item) => item.id === id && item.status === "pending"))));
    } catch (error) {
  ...
  useEffect(() => {
    if (activeItemId) void loadContent(activeItemId);
    else setContent(null);
  }, [activeItemId, loadContent]);

  const pendingIds = useMemo(
    () => review?.items.filter((item) => item.status === "pending").map((item) => item.id) ?? [],
    [review],
  );
  ...
  const toggleAll = () => setSelected(allPendingSelected ? new Set() : new Set(pendingIds));
  ...
  const submitReview = async (action: "approve" | "reject") => {
    ...
    try {
      ...
      setReview(result);
      setSelected(new Set());
      const nextPending = result.items.find((item) => item.status === "pending");
      setActiveItemId(nextPending?.id ?? null);
      ...
      <main className="review-shell">
        <section className="review-hero">
          <div>
            ...
            <h1>爬取数据审核</h1>
            <p>逐条检查清洗正文及来源。只有人工通过的数据才会创建 RAG 入库任务。</p>
          </div>
        ...
        <section className="review-workspace">
          <aside className="review-list-panel">
            <div className="review-list-toolbar">
              <label><input type="checkbox" checked={allPendingSelected} onChange={toggleAll} disabled={!pendingIds.length || !canManage} /> 全选待审核</label>
              ...
              <div className="review-item-list">
                {review.items.map((item, index) => (
                  <article key={item.id} className={`review-item ${activeItemId === item.id ? "active" : ""}`} onClick={() => setActiveItemId(item.id)}>
                    <input type="checkbox" checked={selected.has(item.id)} disabled={!canManage || item.status !== "pending"} onClick={(event) => event.stopPropagation()} onChange={() => toggle(item.id)} />
                    <div>
        ...
        <footer className="review-actions">
          ...
          <div>
            <button type="button" className="reject" disabled={!canManage || !selected.size || Boolean(submitting)} onClick={() => requestReview("reject")}><X size={14} /> 驳回所选</button>
          ...
          <div className="review-confirm-summary">
            ...
            <div>
              ...
              <strong className="review-confirm-result">
                {confirmAction === "approve" ? "提交入库" : "永久驳回"}
              </strong>
            </div>
          </div>
          ...
          <div className="review-confirm-notice">
            ...
            <span>
              {confirmAction === "approve"
                ...
                : "对应的暂存正文将被删除，此操作无法撤销，请确认已完成内容核验。"}
            </span>
          </div>
